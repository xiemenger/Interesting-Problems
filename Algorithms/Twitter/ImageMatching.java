...
public class ImageMatching {
    private static int imageMatching(List<int[]> grid1, List<int[]> grid2) {
		int matches = 0;
		for (int i = 0; i < grid1.size(); i++) {
			for (int j = 0; j < grid1.get(i).length; j++) {
				StringBuilder str1 = new StringBuilder();
				StringBuilder str2 = new StringBuilder();
				dfsSearch(grid1, i, j, str1);
				dfsSearch(grid2, i, j, str2);
				if (str1.length() > 0 && str2.length() > 0 && str1.toString().equals(str2.toString())) {
					matches++;
				}
			}
		}
	...
	private static void dfsSearch(List<int[]> grid, int x, int y, StringBuilder island) {
		if (x < 0 || x >= grid.size() || y < 0 || y >= grid.get(x).length || grid.get(x)[y] != 1) {
			return;
		}
		island.append(x).append(y);
		grid.get(x)[y] = 0;
		dfsSearch(grid, x + 1, y, island);
		dfsSearch(grid, x - 1, y, island);
		dfsSearch(grid, x, y + 1, island);
		dfsSearch(grid, x, y - 1, island);
    }
    
    // -----------------------------------------------------------------
    // Old
    private static int imageMatching2(List<int[]> map1, List<int[]> map2) {
